...
public partial class thiet_ke_in_an : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (((DataView)odsSeal.Select()).Count <= DataPager1.PageSize)
            {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["sci"]))
            {
                ...
                hdnSealNiemPhong.Value = progressTitle(dv[0]["ProductCategoryName"].ToString()) + "-sci-" + dv[0]["ProductCategoryID"].ToString() + ".aspx";
            }
            else
            {
                strTitle = strMetaTitle = "Seal niêm phong";
                strDescription = "";
                strMetaDescription = "";
            }
